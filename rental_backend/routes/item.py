...
async def get_items(user=Depends(UnionAuth(scopes=["rental.item.read"], auto_error=False))) -> list[Item]:
...
async def create_item(item: ItemPost, user=Depends(UnionAuth(scopes=["rental.item.create"], auto_error=False))) -> ItemGet:
...
async def update_item(id: int, is_available: bool, user=Depends(UnionAuth(scopes=["rental.item.update"], auto_error=False))) -> ItemGet:
